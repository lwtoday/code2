...
public class CustomerAction extends BaseAction<Customer> {
	...
	public String sendSms() throws IOException {
		...
		// // 编辑短信内容(使用阿里大于短信平台发送，短信模板是在 阿里后台管理中心进行短信模板配置的）
		// final String msg = "尊敬的用户您好，本次获取的验证码为：" + randomCode
		// + ",服务电话：18672304510。笨蛋才把验证告诉别人！";

		// 调用MQ服务，发送一条消息
		jmsTemplate.send("bos_aliyun_sms", new MessageCreator() {
			@Override
			public Message createMessage(Session session) throws JMSException {
				MapMessage mapMessage = session.createMapMessage();
				mapMessage.setString("recNum", model.getTelephone());
				mapMessage.setString("smsCode", randomCode);
				return mapMessage;
			}
		});
		return NONE;
